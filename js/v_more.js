...
import { pasteBlock } from './sheets.js';
...
function vSync() {
  ...
  return header('общая база', 'Синхронизация', back) + st +
    (ready() ? '' : pasteBlock('🔗 Есть код с другого устройства?')) + setup +
...
function vHelp() {
  ...
  return header('справка', 'Как пользоваться', back) +
    ...
    p('🍏 Иконка на айфоне — отдельное приложение', 'iOS держит для неё своё хранилище: что настроено в Safari, туда не переносится, и ссылка из переписки открывается всё равно в Safari. ' +
      'Поэтому один раз: скопировать ссылку подключения, открыть приложение с иконки и нажать «Вставить из буфера». Дальше иконка работает сама.');
}
